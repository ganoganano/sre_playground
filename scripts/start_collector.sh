...
info "Starting OpenTelemetry Collector, Tempo, and Grafana"
docker compose -f "${COMPOSE_FILE}" up -d
info "Collector started on OTLP gRPC :4317 / HTTP :4318"
info "Tempo API is available on http://localhost:3200"
info "Grafana is available on http://localhost:3300 (admin/admin)"
